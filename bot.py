...
async def p(ctx, keyword, n=5):
    ...
    response = requests.get(url=f'https://api.rule34.xxx//index.php?page=dapi&s=post&q=index&limit=250&json=1&tags={keyword}')
    try:
    ...
    for i in range(n):
        ...
        embed = discord.Embed(title=f'rule34.xxx - {keyword}', color=0x00a8a5)
